...
* ERM (Empirical Risk Minimization): The standard baseline in which the model is trained directly on the available source-domain training data without any explicit domain generalization strategy [12].
* PAIR: Out-of-Domain Robustness via Targeted Augmentations introduces targeted data augmentation designed to improve robustness to specific distribution shifts [8].
* Fish: A gradient-based domain generalization method that uses gradient matching to encourage models trained on different domains to learn more transferable representations [10].
* LISA: A selective augmentation approach that improves OOD robustness by augmenting training data in a targeted manner [11].
* ERM + Target Augmentation: An extension of the standard ERM training procedure that incorporates targeted augmentations to improve robustness to the target-domain distribution [8].
* ContriMix: An unsupervised domain generalization method designed for microscopy images. It disentangles content and domain-specific attributes and uses this separation to generate more domain-robust training examples [9].
* PST (ours): The proposed PhyCV-based approach, which applies the Phase Stretch Transform as a deterministic preprocessing step before the images are provided to the neural network. Unlike the other approaches, PST addresses domain shift primarily at the input representation level, without modifying the model architecture or introducing an additional domain-generalization training objective [1].
