...
During training, images from different hospitals first pass through the PhyCV transformation. The resulting standardized feature maps are then used as inputs to the neural network. The model is trained using standard supervised learning, where predictions are compared against the ground-truth labels and the parameters are updated through backpropagation(Figure 3).

![Figure 4: Training Phase]({{ site.baseurl }}/images/Fig4.png)
*Figure 4. Workflow of the proposed framework, illustrating multi-hospital data standardization using PhyCV followed by deep neural network training via backpropagation.*
...
![Figure 5: OOD Data Partitioning]({{ site.baseurl }}/images/Fig5.png)
*Figure 5. Cross-hospital data partitioning for out-of-distribution (OOD) evaluation, where data from Hospitals 1–3 are used for training, Hospital 4 for validation, and Hospital 5 for testing.*
...
To isolate this effect, the authors take a single microscopic cell image and synthetically generate six progressively more challenging illumination conditions. Each level introduces increasing degradation, ranging from mild brightness variations to severe non-uniform illumination that obscures important visual details. These are visualised in Figure 6.

![Figure 6: Increasing illumination level changes]({{ site.baseurl }}/images/Fig6.png)
*Figure 6. Effect of progressive information loss on a medical image. Image 1 represents the original image (0% information loss), while Images 2–6 illustrate increasing levels of information loss (20%, 40%, 60%, 80%, and 100%, respectively). As the degree of information loss increases, image intensity and structural details are progressively degraded, demonstrating the impact of information degradation on visual quality and the preservation of diagnostically relevant features.*
...
![Figure 7: Robustness of PhyCV under varying illumination conditions.]({{ site.baseurl }}/images/Fig7.png)
*Figure 7. Comparison of image entropy across varying illumination levels with and without PhyCV refinement. The entropy of original images decreases as illumination becomes increasingly non-uniform, indicating progressive information loss. In contrast, PhyCV maintains nearly constant entropy under varying illumination conditions, with a significant drop 6th illumination level(-100%), which can be identified as unreliable.*
...
![Figure 8 : Representation of histopothology images, original and PhyCV Refined]({{ site.baseurl }}/images/Fig8.png)
*Figure 8. Representative histopathology image patches from five different hospitals before and after PhyCV refinement. The top row shows the original H&E-stained images, while the bottom row presents the corresponding PhyCV-refined images, illustrating enhanced structural consistency and reduced staining variability across institutions.*
